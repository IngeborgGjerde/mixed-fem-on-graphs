...
if __name__ == '__main__':
    ...
    kmax = OptDB.getInt('kmax', 9)

    kappa = Constant(OptDB.getScalar('kappa', 1E0))
    
    root = os.path.join('./results', geometry)
    not os.path.exists(root) and os.makedirs(root)

    def get_path(filename, root=root, kappa=kappa(0)):
        return os.path.join(root, '_'.join((f'kappa{kappa}', filename)))
    
    set_log_level(40)
    
    p0 = Expression('sin(k*pi*x[0])', degree=4, k=2)
    u0 = Expression('-(k*pi)*K*cos(k*pi*x[0])', degree=4, k=2, K=kappa(0))
    f = Expression('(k*pi)*(k*pi)*K*sin(k*pi*x[0])', degree=4, k=2, K=kappa(0))
    ...
    for k in range(2, kmax):
        ...
        # FIXME: kappa that varies in space
        A, B, b, W = get_system(mesh, f, p0, kappa=kappa)
        ...
        error_history.append((hmin, ndofs, dimLM, eu, ep))
    ...
    print(tabulate.tabulate(history, headers=('N', 'h', 'lmin', 'lmax', 'cond',
                                              ...
                                              'dimLM', 'nbifs', 'ndofs'), tablefmt='latex'))
    print()
