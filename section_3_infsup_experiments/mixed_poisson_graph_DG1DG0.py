...
# work on interval = (0, L) and we have no multipliers and use DG to get
# the continity
...
def get_system(mesh, f, p0):
    ...
    hK, n = avg(CellDiameter(mesh)), FacetNormal(mesh)
    # Penalty dg
    # FIXME: normal?
    a[0][1] += -inner(avg(p), jump(v))*dS
    a[1][0] += -inner(avg(q), jump(u))*dS

    gamma = Constant(50)
    a[0][0] += (gamma/hK)*inner(jump(u), jump(v))*dS
    
    scale = Constant(assemble(Constant(1)*dx(domain=mesh)))
    
    a_prec = block_form(W, 2)
    a_prec[0][0] = inner(u, v)*dx + inner(v.dx(0), u.dx(0))*dx + (gamma/hK)*inner(jump(u), jump(v))*dS
...
if __name__ == '__main__':
    ...
    for k in range(2, 10):
        ...
        eu = sqrt(assemble(
            ...
            (1/hK)*inner(jump(u0-uh), jump(u0-uh))*dS(metadata={'quadrature_degree': 5})
            + inner(uh.dx(0) - f, uh.dx(0) -f)*dx(metadata={'quadrature_degree': 5})
        ))        

        ep = errornorm(p0, ph, 'L2')
        print(f'|eu|_1 = {eu:.4E} |ep| = {ep:.4E}')
